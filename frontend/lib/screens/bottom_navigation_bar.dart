...
class _BottomNavBarState extends State<BottomNavBar> {
  ...
  @override
  Widget build(BuildContext context) {
    final screenWidth = MediaQuery.of(context).size.width; // Get screen width for responsive layout
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: Transform.translate(
          ...
          child: IconButton(
            icon: Icon(Icons.account_circle),
            iconSize: screenWidth * 0.1, // Responsive icon size based on screen width
            onPressed: () {
              showMyPageModal(context); // 모달 창을 띄우는 함수 호출
            },
          ),
      ...
      bottomNavigationBar: Container(
        ...
        child: BottomNavigationBar(
          ...
          items: <BottomNavigationBarItem>[
            BottomNavigationBarItem(
              icon: Icon(Icons.fitness_center, size: screenWidth * 0.07), // Responsive icon size
              label: '운동',
            ),
            BottomNavigationBarItem(
              icon: Icon(Icons.calendar_month, size: screenWidth * 0.07), // Responsive icon size
              label: '달력',
            ),
            BottomNavigationBarItem(
              icon: Icon(Icons.photo_camera, size: screenWidth * 0.07), // Responsive icon size
              label: '카메라',
            ),
            BottomNavigationBarItem(
              icon: Icon(Icons.timeline, size: screenWidth * 0.07), // Responsive icon size
              label: '그래프',
            ),
          ],
          currentIndex: _selectedIndex,
