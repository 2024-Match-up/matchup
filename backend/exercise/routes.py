...
# ## Hardware 전용
# @router.get("/{exercise_id}/usernames/{nickname}")
...
# @router.get("/count/{session_id}/{count}/hw")
...
#                 prev_cnt = int(rc.get(f"{session_id}_mp_count") or 0)
#                 prev_set = int(rc.get(f"{session_id}_mp_set") or 0)

#                 cur_cnt = metrics.get('counter', 0)
#                 cur_set = metrics.get('sets', 0)
...
#                     # 점수 계산 및 저장
#                     total_count = int(rc.get(f"{session_id}_total_count") or 0)
#                     if cur_cnt != prev_cnt:  # 추가된 조건: 카운트 값이 변경된 경우에만 total_count 증가
#                         total_count += 1
...
@router.get("/count/{session_id}/{count}/hw", summary="HW 카운트 갱신")
async def get_counts(session_id: int, count: int):
    rc.set(f"{session_id}_hw_count", count)
    hw_set = int(rc.get(f"{session_id}_hw_set")) if rc.get(f"{session_id}_hw_set") is not None else 0
