...
def get_waist_joint_points(landmarks):
    left_shoulder = [landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].x,
                     landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].y]
    right_shoulder = [landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].x,
                      landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y]
    ...
    left_arm_angle = dou_calculate_angle(left_shoulder, left_elbow)
    right_arm_angle = dou_calculate_angle(right_shoulder, right_elbow)
    ...
    joint_points = {
        ...
        "hor_shoulder_angle": hor_shoulder_angle,
        "hor_elbow_angle": hor_elbow_angle,
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            landmarks = results.pose_landmarks.landmark
            joint_points = get_waist_joint_points(landmarks)

            hor_shoulder_angle = joint_points["hor_shoulder_angle"]
            hor_elbow_angle = joint_points["hor_elbow_angle"]
            left_arm_angle = joint_points["left_arm_angle"]
            right_arm_angle = joint_points["right_arm_angle"]

            if not resting:
                # 기본1 : 양 팔꿈치가 어깨보다 위에 있으면서
                if joint_points["left_shoulder"][1] > joint_points["left_elbow"][1] and joint_points["right_shoulder"][1] > joint_points["right_elbow"][1]:
                    # 수평 상태(5도 미만)
                    if hor_shoulder_angle <= 5 and hor_elbow_angle <= 5:
                        position = "Ready"
                        # 기본 2 : 좌 / 우 모두 한 번씩 했다면
                        if right_arm_raised and left_arm_raised:
                            ...
                            if counter >= 10:
                                ...
                                rest_start_time = time.time()
                # 왼쪽 아래로 : 왼쪽 팔꿈치가 어깨보다 낮으면서 팔의 각도는 50 이상
                if joint_points["left_elbow"][1] > joint_points["left_shoulder"][1] and left_arm_angle > 30 and hor_shoulder_angle > 30:
                    position = "Left"
                    left_arm_raised = True

                # 오른쪽 아래로 : 오른쪽 팔꿈치가 어깨보다 낮으면서 팔의 각도는 50 이상
                if joint_points["right_elbow"][1] > joint_points["right_shoulder"][1] and right_arm_angle > 30 and hor_shoulder_angle > 30:
                    position = "Right"
                    right_arm_raised = True
            ...
            # 각도 화면에 띄우기
            cv2.putText(image, f"{hor_elbow_angle:.3f}",
                        tuple(np.multiply((joint_points["left_elbow"][0] + joint_points["right_elbow"][0]) / 2, [640, 480]).astype(int)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 255, 255), 2, cv2.LINE_AA)
