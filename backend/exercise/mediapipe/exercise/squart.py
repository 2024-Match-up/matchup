...
class SquatExercise:
    ...
    def read_serial_data(self):
        try:
            if self.ser.in_waiting > 0:
                line = self.ser.readline().decode('utf-8').strip()
                if line.startswith("AngleAcX"):
                    parts = line.split()
                    angleAcX = float(parts[1])
                    count = int(parts[2])
                    return {'angleAcX': angleAcX, 'count': count}
            return None
        except Exception as e:
            logger.error(f"Error reading serial data: {e}")
            return None
    ...
    def calculate_metrics(self, coordinates):
        try:
            hardware_data = self.read_serial_data()
            logger.info(f"Received hardware data: {hardware_data}")
            if hardware_data:
                angleAcX = hardware_data['angleAcX']
                self.counter = hardware_data['count']
                logger.info(f"하드웨어로 인식된 횟수: {self.counter}")
                if self.counter == 10:
                    self.sets += 1
                    self.counter = 0
            ...
            #     self.position = "up"
            #     self.counter += 1
            #     logger.info(f"미디어파이프로 인식된 횟수: {self.counter}")
            #     if self.counter == 10:
            #         self.sets += 1
            #         self.counter = 0
